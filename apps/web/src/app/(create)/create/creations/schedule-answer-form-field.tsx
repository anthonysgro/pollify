...
import { FormField, FormControl, FormLabel, FormDescription, FormItem, FormMessage  } from '@/components/ui/form'
...
// Current strategy: Have a day picker mode where it only deals with whole dates,
// then a free entry mode where people can pick datetimes, overlapping ranges, whatever
    ...
    return (
        <>
            <div className="sm:grid sm:grid-cols-2 sm:gap-6">
                ...
                <Calendar
                    mode="multiple"
                    selected={fields.map((item) => DateTime.fromJSDate(item.from).startOf('day').toJSDate() )}
                    onSelect={(dates) =>
                        handleChangeCalendarDates(dates || [])
                    }
